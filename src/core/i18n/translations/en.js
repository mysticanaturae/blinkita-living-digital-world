/*
==========================================
BLINKITA METHOD™
LIVING DIGITAL WORLD™

ENGLISH LANGUAGE™

Version 1.2
==========================================
*/

export const en = {

    language: {

        name: "English",

        nativeName: "English"

    },


    common: {

        continue: "Continue",

        back: "Back",

        next: "Next",

        begin: "Begin",

        save: "Save",

        choose: "Choose"

    },


    archetypes: {

        explorer: "Explorer"

    },


    creatorStages: {

        beginning: "Beginning",

        creation: "Creation"

    },


    worldBuilder: {

        evolution: {

            title: "World Builder Evolution™",

            currentStage: "Current Stage",

            level: "Level",

            experience: "Experience",

            completeStep: "Complete Creation Step",

            milestones: "Milestones",

            defaultEssence:
                "Your world is awakening."

        },


        domains: {

            title: "Choose Your Creation Path™",

            introduction:
                "Every World Builder™ begins by choosing what kind of living creation wants to emerge.",

            choose: "Choose",


            digitalWorlds: {

                name: "Living Digital Worlds™",

                description:
                    "Applications, platforms, AI systems, SaaS, portals and digital ecosystems."

            },


            books: {

                name: "Living Books™",

                description:
                    "Books, series, educational materials and interactive publications."

            },


            businesses: {

                name: "Living Businesses™",

                description:
                    "Companies, brands, services and value creating systems."

            },


            academies: {

                name: "Living Academies™",

                description:
                    "Educational systems, programs, schools and learning journeys."

            },


            communities: {

                name: "Living Communities™",

                description:
                    "Communities, memberships, movements and human networks."

            },


            creativeProjects: {

                name: "Living Creative Projects™",

                description:
                    "Art, music, films, exhibitions and creative experiences."

            },


            organizations: {

                name: "Living Organizations™",

                description:
                    "Foundations, NGOs, public projects and collective initiatives."

            }

        },


        stages: {

            dreamer: {

                name: "Dreamer™",

                essence:
                    "The vision of a new world begins."

            },

            explorer: {

                name: "Explorer™",

                essence:
                    "The creator discovers possibilities."

            },

            creator: {

                name: "Creator™",

                essence:
                    "Ideas become living creations."

            },

            worldBuilder: {

                name: "World Builder™",

                essence:
                    "The creator designs living ecosystems."

            },

            worldArchitect: {

                name: "World Architect™",

                essence:
                    "The creator shapes complete worlds."

            },

            livingCreator: {

                name: "Living Creator™",

                essence:
                    "Creation becomes a living intelligence."

            },

            legacyBuilder: {

                name: "Legacy Builder™",

                essence:
                    "The world continues beyond the creator."

            }

        }

    },


    dashboard: {

        welcome: "Welcome to your Living World™",

        introduction:
            "This is your living space for creation, memory, journey and evolution.",

        creatorWelcome:
            "Welcome, Creator™",

        creatorIntroduction:
            "Your evolution space inside the Living World™",

        creatorIdentity:
            "Creator Identity™",

        creatorIdentityPrompt:
            "Tell your Living World™ who you are.",

        name:
            "Name",

        yourName:
            "Your name",

        archetype:
            "Creator Archetype",

        archetypePlaceholder:
            "Explorer, Artist, Builder...",

        aboutYou:
            "About You",

        aboutYouPlaceholder:
            "Tell your Living World™ something about you...",

        saveIdentity:
            "Save My Identity",

        identitySaved:
            "Identity Saved ✨",

        anonymousCreator:
            "Anonymous Creator",

        explorer:
            "Explorer",

        creatorIdentitySummary:
            "Creator Identity",

        archetypeSummary:
            "Archetype",

        about:
            "About",

        creatorJourney:
            "Creator Journey™",

        level:
            "Level",

        experience:
            "Experience",

        stage:
            "Stage",

        xp:
            "XP",

        livingWorldStatus:
            "Living World™ Status",

        currentPortal:
            "Current Portal",

        evolution:
            "Evolution",

        decisions:
            "Decisions",

        governance:
            "Governance",

        evaluations:
            "evaluations",

        actions:
            "Actions",

        myLivingWorlds:
            "My Living Worlds™",

        worldStatus:
            "Status",

        firstLivingWorldWaiting:
            "Your first Living World™ is waiting.",

        achievements:
            "Achievements™",

        achievementsDescription:
            "Your World Builder™ milestones will appear here."

    },

    portal: {

        experiences: {

            arrival: {

                identity:
                    "The First Portal",

                identityMessage:
                    "Every journey begins with a moment of arrival — a moment when you choose to be present.",

                atmosphere:
                    "Opening",

                atmosphereMessage:
                    "There is nothing you need to know yet.\nYou only need to arrive.",

                whisper:
                    "Time has been waiting for your arrival.\n\nPerhaps you have been waiting for this moment too.",

                message:
                    "You have arrived.\n\nThis is the beginning of your Living World™ journey.",

                ritual:
                    "Pause for one conscious breath.\n\nLeave everything else outside for a moment.\nBe here.",

                question:
                    "Who are you, and what brings you here?",

                placeholder:
                    "Tell your Living World™ something about yourself.\nThere is no right answer. This is simply the first memory it will have of you.",

                interaction:
                    "Begin Your Journey™"

            }

        },


        experiences: {

            arrival: {

                identity:
                    "The First Portal",

                identityMessage:
                    "Every journey begins with a moment of arrival — a moment when you choose to be present.",

                atmosphere:
                    "Opening",

                atmosphereMessage:
                    "There is nothing you need to know yet.\nYou only need to arrive.",

                whisper:
                    "Time has been waiting for your arrival.\n\nPerhaps you have been waiting for this moment too.",

                message:
                    "You have arrived.\n\nThis is the beginning of your Living World™ journey.",

                ritual:
                    "Pause for one conscious breath.\n\nLeave everything else outside for a moment.\nBe here.",

                question:
                    "Who are you, and what brings you here?",

                placeholder:
                    "Tell your Living World™ something about yourself.\nThere is no right answer. This is simply the first memory it will have of you.",

                interaction:
                    "Begin Your Journey™"

            }

        },


        atmosphere:
            "Atmosphere",

        whisper:
            "Whisper",

        message:
            "Message",

        ritual:
            "Ritual",

        firstQuestion:
            "Your First Question",

        continueJourney:
            "Continue Journey",

        beginJourney:
            "Begin Your Journey™",

        placeholder:
            "Write your answer here...",

        responses: {

            arrival: {

                title:
                    "Your Arrival",

                message:
                    "Thank you.\n\nYour first words are now part of your Living World™ memory.\n\nThis is the first thing your world knows about you."

            },

            invitation: {

                title:
                    "The Invitation Is Open",

                message:
                    "You have accepted the invitation.\n\nSomething has begun to move.\n\nYour Living World™ now knows what is asking for your attention."

            },

            possibility: {

                title:
                    "A Possibility Has Appeared",

                message:
                    "You have named a possibility.\n\nIt does not need to exist yet.\n\nIt only needed a place to begin."

            },

            theCall: {

                title:
                    "You Heard the Call",

                message:
                    "You listened.\n\nYour answer gives the call its first form.\n\nYou do not need to know the whole path yet."

            },

            worldSeed: {

                title:
                    "The Seed Has Been Planted",

                message:
                    "Your world now has its first seed.\n\nIt is small enough to grow.\n\nAnd alive enough to become something more."

            },

            vision: {

                title:
                    "Your Vision Has Been Remembered",

                message:
                    "You have given your world a first glimpse of what it could become.\n\nYour vision can change as your world grows.\n\nThat is part of being alive."

            },

            essence: {

                title:
                    "The Essence Has Been Found",

                message:
                    "You have named something that belongs at the heart of your world.\n\nThis essence can become the thread that connects everything that follows."

            },

            experience: {

                title:
                    "The Experience Has Begun",

                message:
                    "You have imagined what it means to enter your world.\n\nNow it has something to offer — not only as an idea, but as an experience."

            },

            livingWorld: {

                title:
                    "Your Living World Has Begun",

                message:
                    "Look at what you have created.\n\nYour answers are now part of its first memory.\n\nThis is not the end of the journey.\n\nThis is where your Living World™ begins to evolve."

            },

            remembered: {

                title:
                    "Your World Has Remembered",

                message:
                    "Your answer has become part of your Living World™ memory."

            }

        }

    }
};

